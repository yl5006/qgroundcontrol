...
{
    Q_UNUSED(errorCode);
    qgcApp()->showMessage(tr("任务上传下载错误: %1").arg(errorMsg));//Mission transfer failed. Retry transfer. Error: %1"
}
...
void Vehicle::_connectionLostTimeout(void)
{
    if (_connectionLostEnabled && !_connectionLost) {
        ...
        _say(QString("%1 连接丢失").arg(_vehicleIdSpeech()));
        if (_autoDisconnect) {
...
void Vehicle::_connectionActive(void)
{
    ...
    if (_connectionLost) {
        ...
        _say(QString("%1 重新连接").arg(_vehicleIdSpeech()));
