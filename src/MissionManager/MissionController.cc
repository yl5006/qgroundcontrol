...
const char* MissionController::_jsonFileTypeValue =             "EWT_Mission";
...
void MissionController::applyoffboardmission(void)
{
    ...
    for (int i=1; i<rgMissionItems.count(); i++) {
       SimpleMissionItem * newItem =new SimpleMissionItem(_controllerVehicle, _flyView , *rgMissionItems[i],this);
...
int MissionController::insertSimpleMissionItem(QGeoCoordinate coordinate, int i)
{
    ...
    if (newItem->specifiesAltitude()) {
        ...
        if (_findPreviousAltitude(i, &prevAltitude,&prevSpeed, &prevAltitudeMode)) {
    ...
    for (int i=0; i<visualItems->count(); i++) {
        if (visualItems->value<VisualMissionItem*>(i)->isSimpleItem()) {
            ...
            if (doJumpItem->command() == MAV_CMD_DO_JUMP) {
                bool found = false;
                int findDoJumpId = static_cast<int>(doJumpItem->missionItem().param8());
    ...
    if (!plannedHomePositionInFile) {
        ...
        for (int i=1; i<visualItems->count(); i++) {
            ...
            if (item && item->command() == MAV_CMD_DO_JUMP) {
                item->missionItem().setParam8(static_cast<int>(item->missionItem().param8()) + 1);
            }
        }
    }
...
void MissionController::loadFromTxtFile(const QString& filename,double angle,double space,double addalt,int waynum,bool cammer,bool relalt)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        {
            SimpleMissionItem* item = new SimpleMissionItem(_controllerVehicle,_flyView, this);
...
void MissionController::_recalcWaypointLines(void)
{
    ...
    for (int i=1; i<_visualItems->count(); i++) {
        ...
        if(item->isSimpleItem() &&(qobject_cast<SimpleMissionItem*>(item)->command() == MAV_CMD_DO_JUMP))
        {
...
bool MissionController::_findPreviousAltitude(int newIndex, double* prevAltitude, double* prevSpeed, int* prevAltitudeMode)
{
    bool        found = false;
